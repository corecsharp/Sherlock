...
using Sherlock.Framework.Data.Conventions;
    ...
    public sealed class DapperRuntime
    {
            ...
            IEnumerable<IDapperMetadataProvider> metadataProviders)
        {
            ...
            catch (ArgumentException exception)
            {
                ...
                throw new SherlockException($"{duplicate?.EntityType} 存在重复的 IDapperMetadataProvider ,请检查", exception);
            }
        ...
        private DapperDataSource NewDataSource(string readingConnectionName, string writingConnectionName)
        {
            ...
            {
                throw new SherlockException($"数据库连接 {writingConnectionName} 和 {readingConnectionName} 配置了不同的 {nameof(IDatabaseProvider)}，读写库必须使用一致的提供程序。");
            }
