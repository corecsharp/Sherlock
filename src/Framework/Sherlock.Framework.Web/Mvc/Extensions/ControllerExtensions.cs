...
namespace Sherlock.Framework.Web.Mvc
{
    public static class ControllerExtensions
    {
        ...
        public static IActionResult HandldError(this Controller controller, string viewName = "Error", Func<Exception, Exception> handler = null)
        {
            ...
            var message = SherlockEngine.Current.IsDevelopmentEnvironment.IfNull(false) ?
            ...
            return SherlockEngine.Current.IsDevelopmentEnvironment.IfNull(false)
                ? (IActionResult)controller.Content(message)
                : controller.StatusCode(500);
        }

        public static IActionResult HandldError(this SherlockApiController controller, Func<Exception, Exception> handler = null)
        {
            var feature = controller.HttpContext.Features.Get<IExceptionHandlerFeature>();
            var error = feature?.Error;
