...
namespace Sherlock.Framework.Web.FileProviders
{
    /// <summary>
    ...
    public class ModuleFileProvider : IFileProvider, IDisposable
    {
        ...
        private void EnsureInit()
        {
            if (_providers == null)
            {
                lock (this)
                {
                    if (_providers == null)
                    {
                        IModuleManager manager = (IModuleManager)SherlockEngine.Current.GetService(typeof(IModuleManager));
        ...
        private IFileProvider ParseFileProvider(string subPath, out string path)
        {
            ...
            if (match.Success)
            {
                ...
                {
                    throw new SherlockException($"找不到模块 {moduleName}。");
                }
            }
            else
            {
