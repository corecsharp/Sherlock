...
namespace Sherlock.Framework.Scheduling
{
        ...
        where TJob : IScheduledJob
    {
        ...
        {
            var fac = SherlockEngine.Current.GetRequiredService<IServiceScopeFactory>();
            var logger = SherlockEngine.Current.GetService<ILoggerFactory>()?.CreateLogger(typeof(TJob));
            var tracing = SherlockEngine.Current.GetRequiredService<IOptions<SchedulingOptions>>().Value?.EnableJobExecutionTracing ?? true;
