...
namespace Sherlock.Framework.Services
{
    ...
    public class SnowflakeIdGenerationService : IIdGenerationService
    {
        ...
        public SnowflakeIdGenerationService(
            ...
            ISherlockEnvironment SherlockEnvironment, 
            ILoggerFactory loggerFactory)
        {
            Guard.ArgumentNotNull(networkOptions, nameof(networkOptions));
            Guard.ArgumentNotNull(SherlockEnvironment, nameof(SherlockEnvironment));
            ...
            if ((networkOptions.Value.DataCenterId <= 0 || lanOptions == null) && !SherlockEnvironment.IsDevelopmentEnvironment)
            {
                throw new InvalidOperationException($"未能从 Sherlock:Network 中查找到合法的 DataCenterId，" +
            ...
            {
                if (!SherlockEnvironment.IsDevelopmentEnvironment)
                {
                    throw new SherlockException($"Snowflake 唯一 Id 生成配置错误，必须保证 {nameof(NetworkOptions)} 至少配置了一个{nameof(NetworkOptions.Lans)}，且与本机 Id 地址匹配。");
                }
                else
                {
