...
namespace Sherlock.Framework.Events
{
    ///<summary>
    /// 定义一个事件的基类。
    ///</summary>
    public abstract class EventBase
    {
        ...
        private void ExecuteAndReleaseHandler(object sender, object args, EventSubscriptionDescriptor s)
        {
            ...
            try
            {
                ...
                {
                    throw new SherlockException($@"事件订阅方法中的事件参数和事件发布的参数不匹配。{System.Environment.NewLine} 事件 ""{s.EventName}"" 要求订阅方法 {s.DependencyType}.{s.MethodName} 签名必须为 void (object sender, {args?.GetType() ?? typeof(object)})");
                }
