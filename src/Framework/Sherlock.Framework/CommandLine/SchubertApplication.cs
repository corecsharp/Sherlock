...
{
    public static class SherlockApplication
    {
            ...
            where T : CommandLineStartup, new()
        {
            ...
            serviceCollection.AddSherlockFramework(startup.Configuration, startup.ConfigureServices, startup.ConfigureShellCreationScope);

            IServiceProvider serviceProvider = serviceCollection.BuildServiceProvider();
            startup.Configure(serviceProvider);

            serviceProvider.StartSherlockEngine();
        ...
        private static void ExecuteStartup<T>(T startup, IServiceProvider serviceProvider) where T : CommandLineStartup, new()
        {
            ...
            if (methodInfo == null)
            {
                throw new SherlockException($"CommandLineStartup 实现类 {t.Name} 必须具有名为 Run 的实例方法才能启动。");
            }
            ...
            if ((parameters?.Length ?? 0) > 0)
            {
                ...
                for (int i = 0; i < parameters.Length; i++)
                {
                    ...
                    {
                        throw new SherlockException($"CommandLineStartup 实现类 {t.Name} 的 Run 方法参数 {t.Name} 不是 DI 中的依赖项。");
                    }
                    else
                    {
                        try
        ...
        private static void BuildConfiguration<T>(T startup, params string[] args) where T : CommandLineStartup, new()
        {
            var builder = new ConfigurationBuilder()
                            .SetBasePath(SherlockUtility.GetApplicationDirectory())
                            .AddJsonFile("appsettings.json", optional: true, reloadOnChange: true);
            String env = "production";
            var section = builder.Build().GetSection("Sherlock:Env");
            if (section != null)
            {
