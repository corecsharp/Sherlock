...
namespace Sherlock.Framework.Environment.Modules
{
    public class FeatureManager : IFeatureManager
    {
        ...
        {
            var attributes = type.GetTypeInfo().GetCustomAttributes(typeof(SherlockFeatureAttribute), false);
            foreach (SherlockFeatureAttribute featureAttribute in attributes)
            {
                    ...
                    (currentFeatureName, featuresState) =>
                    {
                        ...
                        return feature.Key.Dependencies
                                      .Select(fName =>
                                      {
                                          ...
                                          if (states.Count == 0)
                                          {
                                              throw new SherlockException("Failed to get state for feature {0}", fName);
                                          }

                                          if (states.Count > 1)
                                          {
                                              throw new SherlockException("Found {0} states for feature {1}", states.Count, fName);
                                          }
