...
namespace Sherlock.Framework.Environment.ShellBuilders
{
    /// <summary>
    ...
    public class ShellContextFactory : IShellContextFactory
    {
        ...
        private SherlockOptions _options;
            ...
            foreach (var m in methods)
            {
                if (m.GetCustomAttributes<EventSubscriptionAttribute>().Any())
                {
                    foreach (var descriptor in EventSubscriptionDescriptor.FromMethod(dependencyType, lifetime, m))
                    {
                        ...
                        {
                            throw new SherlockException($"方法 {m.DeclaringType.FullName}.{m.Name} 存在不明确的或重复的事件订阅声明，请考虑使用 {nameof(EventSubscriptionAttribute)}.{nameof(EventSubscriptionAttribute.DeclaringDependencyType)} 属性标识。");
                        }
                    }
                }
            }
        ...
        private static void SetupServiceDescriptors(ShellBlueprint blueprint, List<SmartServiceDescriptor> smartServiceDescriptors)
        {
            foreach (var item in blueprint.DependencyDescribers)
            {
                ...
                if (constructor == null)
                {
                    throw new SherlockException($@"类型 {item.Type.FullName} 实现了 {nameof(IDependencyDescriber)} 接口，必须提供公共无参构造函数。");
                }
        ...
        private ShellDescriptor MinimumShellDescriptor()
        {
            ...
            {
                ApplicationName = _options.AppSystemName.IfNullOrWhiteSpace("SherlockApp"),
